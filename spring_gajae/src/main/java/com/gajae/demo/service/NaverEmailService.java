...
public class NaverEmailService {
    ...
    public void sendSubscriptionMessage(String recipientEmail) throws MessagingException {
        MimeMessage mimeMessage = mailSender.createMimeMessage();
        MimeMessageHelper messageHelper = new MimeMessageHelper(mimeMessage, true, "UTF-8");
        messageHelper.setFrom("loveseesee@naver.com");
        messageHelper.setTo(recipientEmail);
        messageHelper.setSubject("GAJAE.COM 사이트 구독되었습니다.");
        messageHelper.setText("구독이 완료되었습니다.", true);
        mailSender.send(mimeMessage);
    }
}