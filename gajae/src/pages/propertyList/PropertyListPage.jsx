...
const PropertyListPage = ( ) => {
  ...
  useEffect(() => {
    if (orderBy === '가격 낮은순') {
      axios.post('http://localhost:9999/search/list', { orderBy: 'priceLow', P_ADDRESS, ROOM_CAPACITY})
        .then(response => {
          setProperty(response.data);
        }) 
        .catch(error => {
      ...
      axios.post('http://localhost:9999/search/list', { orderBy: 'priceHigh', P_ADDRESS, ROOM_CAPACITY})
      .then(response => {
        setProperty(response.data);
      }) 
      .catch(error => {
  ...
  const ranks  = (P_STAR, value) => {
    if('1' === P_STAR){ 
      axios.post('http://localhost:9999/search/list', {P_ADDRESS, ROOM_CAPACITY, P_STAR})
      .then(response => {
        ...
        <div className="container" style={{ display: 'flex', justifyContent: 'center' }}>
          <div className="row">
            ...
            <div className="col-lg-9 col-md-12">
                  <h4 className="search-hotel" style={{marginTop:"20px", textAlign:"center", fontWeight: "bold"}}>{params.P_ADDRESS} : 검색된 숙소{property.length}개</h4>
                <DropdownButton id='dropdown-btn' title={orderBy ? orderBy : '정렬 순서'}>
                  <Dropdown.Item id='dropdownItem-btn' onClick={() => handleOrder('가격 낮은순')}>가격 낮은순</Dropdown.Item>
                  <Dropdown.Item id='dropdownItem-btn' onClick={() => handleOrder('가격 높은순')}>가격 높은순</Dropdown.Item>
                </DropdownButton>
